...
async function startalpha() {
   ...
   global.loadDatabase = async function loadDatabase() {
      if (global.db._read) await global.db._read
      if (global.db.data !== null) return global.db.data
      global.db._read = global.db.read().catch(console.error)
      await global.db._read
      global.db.data = {
         users: {},
         chats: {},
         database: {},
   ...
   const alpha = alphaConnect({
      ...
      browser: ['Bot By KodratBotWa', 'safari', '1.0.0'],
      auth: state
   })
...
const { isSetLeft, getTextSetLeft } = require('./lib/setleft')
   ...
   alpha.ev.on("viewOnceMessage", async (anu) => {
      const {
         oneTime
      } = require("./lib/welcome");
